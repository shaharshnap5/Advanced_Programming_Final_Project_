...
async def test_create_active_ride_with_explicit_start_time(test_db):
    ...
    cursor = await test_db.execute(
        "SELECT * FROM rides WHERE ride_id = ?", ("RIDE001",)
    )
...
async def test_create_active_ride_without_explicit_start_time(test_db):
    ...
    cursor = await test_db.execute(
        "SELECT * FROM rides WHERE ride_id = ?", ("RIDE002",)
    )
    row = await cursor.fetchone()
    assert row is not None
...
async def test_create_active_ride_degraded_flag_default(test_db):
    ...
    cursor = await test_db.execute(
        "SELECT is_degraded_report FROM rides WHERE ride_id = ?", ("RIDE005",)
    )
    row = await cursor.fetchone()
...
async def test_create_active_ride_with_specific_timestamp(test_db):
    ...
    cursor = await test_db.execute(
        "SELECT start_time FROM rides WHERE ride_id = ?", ("RIDE200",)
    )
    row = await cursor.fetchone()
...
async def test_get_active_user_ids_returns_only_incomplete_rides(test_db):
    ...
    await test_db.execute(
        (
            "INSERT INTO users (user_id, first_name, last_name, email, payment_token) "
            "VALUES (?, ?, ?, ?, ?)"
        ),
    ...
    await test_db.execute(
        (
            "INSERT INTO rides (ride_id, user_id, vehicle_id, start_station_id, end_station_id, start_time, end_time) "
...
async def test_get_active_ride_by_user_returns_ride_object(test_db):
    ...
    assert active_ride.is_degraded_report is False
...
async def test_get_active_ride_by_user_ignores_completed_rides(test_db):
    ...
    await test_db.execute(
        "UPDATE rides SET end_time = ? WHERE ride_id = ?", (end_time, "RIDE_ACTIVE_002")
    )
    await test_db.commit()

    # Get active ride should return None because the ride is completed
    active_ride = await repo.get_active_ride_by_user(
        test_db, "USER_WITH_COMPLETED_RIDE"
    )
